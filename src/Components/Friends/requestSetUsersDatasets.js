...
export const requestSetUsersDatasets = (itemsPerSet, currentSetOfUsers, setUsersDatasets, setTotalNumberOfUsers) => {
    ...
    get(`${apiWithoutParams}?count=${itemsPerSet}&page=${currentSetOfUsers}`)
        .then(
            response => {
                ...
                setTotalNumberOfUsers(response.data.totalCount)
            })
}